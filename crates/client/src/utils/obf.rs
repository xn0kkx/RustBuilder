...
pub fn run(argument: Args) -> std::io::Result<()> {
    let file = argument.file;
    let technique = argument.technique;
    let operation = argument.operation;
    debug_log(format!(
        "starting obfuscation operation: file={} technique={technique:?} operation={operation:?}",
        file
    ));
    ...
    debug_log(format!(
        "obfuscation input loaded: file={} bytes={}",
        file,
        buffer.len()
    ));
    ...
    match operation {
        Operation::Obfuscate => {
            let output_file = match technique {
                Obfuscation::IPV4 => "obfuscated_ipv4.txt",
                Obfuscation::IPV6 => "obfuscated_ipv6.txt",
                Obfuscation::MAC => "obfuscated_mac.txt",
                Obfuscation::UUID => "obfuscated_uuid.txt",
            };
            debug_log(format!(
                "writing obfuscated output: technique={technique:?} output={output_file}"
            ));

            match technique {
                Obfuscation::IPV4 => {
                    obfuscate_ipv4(&mut buffer, output_file);
                }
                Obfuscation::IPV6 => {
                    obfuscate_ipv6(&mut buffer, output_file);
                },
                Obfuscation::MAC => {
                    obfuscate_mac(&mut buffer, output_file);
                }
                Obfuscation::UUID => {
                    obfuscate_uuid(&mut buffer, output_file);
                }
            }
            debug_log(format!(
                "obfuscation completed successfully: output={output_file}"
            ));
            println!("Obfuscation successful, output written to file.");
        }
        Operation::Deobfuscate => {
            debug_log(format!(
                "reading deobfuscation input: technique={technique:?} file={file}"
            ));
            ...
            match result {
                Ok(deobfuscated) => {
                    let output_path = "deobfuscated_output.bin";
                    let mut output_file = File::create(output_path)?;
                    output_file.write_all(&deobfuscated)?;
                    debug_log(format!(
                        "deobfuscation completed successfully: output={output_path} bytes={}",
                        deobfuscated.len()
                    ));
                ...
                Err(_) => {
                    debug_log(format!(
                        "deobfuscation failed: technique={technique:?} file={file}"
                    ));
